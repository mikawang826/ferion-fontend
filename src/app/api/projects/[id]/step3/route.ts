...
export async function PUT(req: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = assetDetailsSchema.safeParse(json);
    if (!parsed.success) {
    ...
    const updated = await prisma.project.update({
      where: { id: project.id },
      data: {
        assetLocation: data.assetLocation.trim(),
        assetDescription: data.assetDescription.trim(),
        assetValue: data.assetValue,
        currentStep: project.currentStep < 4 ? 4 : project.currentStep,
        updatedBy: user.id,
      },
    });
    ...
    return NextResponse.json(
      { error: "Failed to save asset details" },
      { status: 500 },
    );
  }
}
